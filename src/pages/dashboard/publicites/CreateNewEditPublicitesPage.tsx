...
import {
  createPubliciteAdmin,
  filesPublicitesLogo,
  getPublicite,
  updatePubliciteAdmin,
...
export default function CreateNewEditPublicitePage() {
  ...
  useEffect(() => {
    const fetchPublicite = async () => {
      if (id) {
        const response = await dispatch(getPublicite(id));
        const publicite = response?.payload?.data;
        ...
        if (publicite) {
          setInitialValues({
            ...
            startDate: publicite.startDate?.slice(0, 10) || '',
            endDate: publicite.endDate?.slice(0, 10) || '',
            redirectionLink: publicite.redirectionLink || ''
          });

          setFileName(publicite.image?.split('/').pop() || null);
        }
      }
    };
    ...
    onSubmit: async (values) => {
      try {
        if (values.image) {
          ...
          const response: any = await dispatch(filesPublicitesLogo(formDataFile));
        ...
        if (id) {
          await dispatch(updatePubliciteAdmin({ id, dataForm: values }));
        } else {
          await dispatch(createPubliciteAdmin(values));
        }

        router.push(PATH_DASHBOARD.publicites);
      } catch (error) {
        // Gérer l'erreur ici
      }
    }
  ...
  return (
    <PageContainer menu={id ? "Modifier une publicité" : "Ajouter une publicité"}>
      <Card sx={{ p: 2 }}>
